...
@section('content')
        ...
        <div class="flex flex-col md:flex-row md:items-start md:justify-between gap-4">
            <div>
                ...
                <div class="mt-2 text-sm text-slate-600 flex flex-wrap gap-x-4 gap-y-1">
                    <div><span class="text-slate-500">Created:</span>
                        {{ optional($question->created_at)->format('Y-m-d H:i') }}</div>
                    ...
                    @if ($question->published_at)
                        <div><span class="text-slate-500">Published:</span>
                            {{ optional($question->published_at)->format('Y-m-d H:i') }}</div>
                    @endif
                </div>
            </div>
    ...
    @php
        ...
        $logs = \App\Models\MessageLog::query()->where('question_id', $question->id)->latest()->limit(10)->get();
                        ...
                        @forelse($logs as $log)
                            <tr class="align-top">
                                ...
                                <td class="py-3 pr-4">
                                    @if ($log->status === 'sent')
                                        <span class="qa-badge"
                                            style="background:#ecfdf5;color:#065f46;border:1px solid #a7f3d0;">sent</span>
                                    @elseif($log->status === 'failed')
                                        <span class="qa-badge"
                                            style="background:#fff1f2;color:#9f1239;border:1px solid #fecdd3;">failed</span>
                                    @else
    ...
    <div class="mt-4 grid grid-cols-1 lg:grid-cols-12 gap-4">
            ...
            <div class="qa-card mt-4">
                <div class="flex items-start justify-between gap-3">
                    <div>
                        ...
                        <p class="text-sm text-slate-600 mt-1">ভুল থাকলে শিরোনাম/বডি ঠিক করুন এবং category পরিবর্তন করুন।
                        </p>
                    </div>
                </div>

                {{-- NOTE: এই ফর্ম আলাদা করে “Save Question Changes” দিবে --}}
                <form id="qaQuestionForm" method="POST" action="{{ route('admin.questions.update', $question) }}"
                    class="mt-4">
                    @csrf
                    @method('PUT')

                    <label class="text-xs text-slate-500">Title</label>
                    <input id="qa_question_title" name="title" value="{{ old('title', $question->title) }}"
                        class="qa-input w-full" placeholder="প্রশ্নের শিরোনাম" />

                    <div class="mt-3 grid grid-cols-1 md:grid-cols-3 gap-3">
                        <div>
                            <label class="text-xs text-slate-500">Asker Name</label>
                            <input name="asker_name" value="{{ old('asker_name', $question->asker_name) }}"
                                class="qa-input w-full" placeholder="e.g. Abdullah" />
                        </div>

                        <div>
                            <label class="text-xs text-slate-500">Asker Phone</label>
                            <input name="asker_phone" value="{{ old('asker_phone', $question->asker_phone) }}"
                                class="qa-input w-full" placeholder="e.g. 01XXXXXXXXX" />
                        </div>

                        <div>
                            <label class="text-xs text-slate-500">Asker Email</label>
                            <input name="asker_email" value="{{ old('asker_email', $question->asker_email) }}"
                                class="qa-input w-full" placeholder="e.g. user@email.com" />
                        </div>
                    </div>
                    ...
                    <div class="mt-3">
                        ...
                        <textarea id="question_body_html" name="body_html" class="qa-input w-full min-h-[220px]"
        ...
        <div class="lg:col-span-7">
            <div class="qa-card">
                        ...
                        <div class="text-right">
                            ...
                            @if ($question->answer->answeredBy)
                                <div class="text-xs text-slate-500 mt-1">
                                    By: <span
                                        class="font-semibold text-slate-700">{{ $question->answer->answeredBy->name }}</span>
                                </div>
                            @endif
                        </div>
                    @endif
                ...
                <form id="qaAnswerForm" method="POST" action="{{ route('admin.answers.draft', $question) }}"
                    class="mt-4">
                    @csrf
                    ...
                    <div class="mt-4 flex flex-col sm:flex-row gap-2">
                        ...
                        <button type="submit" class="qa-btn qa-btn-primary w-full sm:w-auto"
                    ...
                    @if ($question->status === 'published')
                        <div class="mt-3 text-sm text-emerald-700">
                            এই প্রশ্নটি published। আপনি চাইলে answer edit করে আবার Publish দিতে পারেন (আপনার policy
                            অনুযায়ী)।
                        </div>
                    @endif
                </form>
